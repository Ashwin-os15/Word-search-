...
export const ALL_FRIENDS = [
  ...
  "VAAJU","ROWDY","PICHI","TANU","ASHWIN",
  // extra south indian names
  "KAVYA","DEEPIKA","HARISH","VISHNU","PRIYA","ARUN","MEENA","RAVI",
  "KEERTHI","SURYA","AARAV","DIVYA","SANJAY","POOJA","VIKRAM","LAKSHMI",
];

// Pre-shuffle and batch friends into groups of 7
function chunkFriends() {
  const all = [...ALL_FRIENDS].map(n => n.toUpperCase().replace(/\s/g,''));
  const batches = [];
  for (let i = 0; i < all.length; i += 7) {
    const batch = all.slice(i, i + 7).filter(n => n.length >= 3 && n.length <= 12);
